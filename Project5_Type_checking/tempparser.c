...
void compileForSt(void) {
  ...
  eat(SB_ASSIGN);
  Type* rhsType1 = compileExpression();

  eat(KW_TO);
  Type* rhsType2 = compileExpression();

  checkTypeEquality(obj->varAttrs->type, rhsType1);
  checkTypeEquality(obj->varAttrs->type, rhsType2);
...
void compileArgument(Object* param) {
  Type* argType = NULL;
  int argStartLineNo = lookAhead->lineNo;
  int argStartColNo = lookAhead->colNo;

  if (param == NULL) {
    error(ERR_PARAMETERS_ARGUMENTS_INCONSISTENCY, argStartLineNo, argStartColNo);
    return;
  }

  if (param->paramAttrs->kind == PARAM_REFERENCE) {
    argType = compileLValue();
    if (currentToken->tokenType == TK_IDENT) {
        Object* lvalueBaseObject = lookupObject(currentToken->string);
        if (lvalueBaseObject != NULL) {
            if (lvalueBaseObject->kind == OBJ_FUNCTION) {
                error(ERR_TYPE_INCONSISTENCY, currentToken->lineNo, currentToken->colNo);
                argType = NULL;
            }
        }
  ...
  if (argType != NULL && param->paramAttrs->type != NULL) {
      if (!compareType(argType, param->paramAttrs->type)) {
        error(ERR_TYPE_INCONSISTENCY, argStartLineNo, argStartColNo);
      }
  }
}
...
void compileArguments(ObjectNode* paramList) {
  ...
  case SB_LPAR:
    ...
    param = paramList != NULL ? paramList->object : NULL;
    compileArgument(param);
    if (paramList != NULL) paramList = paramList->next;
